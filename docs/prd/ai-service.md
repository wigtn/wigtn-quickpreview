...
- 시청 가치 점수 (1-10)
- 점수 근거
- 핵심 키워드 (5-10개, 스크립트에서 자주 언급되는 개념)
- 하이라이트 구간 (주제 전환 기준, 유동적 개수)

**타임스탬프 처리**:
| 항목 | 설명 |
|------|------|
| 입력 형식 | `[N초] 텍스트` (예: `[120초] 안녕하세요`) |
| 출력 형식 | 초 단위 정수 (예: `120`) |
| 검증 | LLM 응답 타임스탬프를 실제 세그먼트와 비교 |
| 보정 | 10초 이상 오차 시 가장 가까운 세그먼트로 보정 |

**하이라이트 규칙**:
- 기준: 주제가 전환되는 구간을 챕터처럼 선정
- 개수: 실제 주제 전환 횟수에 맞게 자유롭게 결정 (2개~7개+)
- 타임스탬프: 반드시 스크립트에 존재하는 시간만 사용

**다국어 처리**:
- 영상 언어와 무관하게 모든 응답은 한국어로 작성
- summary, keywords, highlights의 title/description 모두 한국어
